...
const BusinessDash = () => {
  ...
  const [formData, setFormData] = useState<{
    ...
    type: "",
    name: "", // Business name is required for Business plan
  ...
  // Filter columns - Business plan doesn't show premium data fields
  const visibleColumns = React.useMemo(() => {
    return columns.filter(col => {
      const colId = (col as any).id || (col as any).accessorKey;
      // Hide premium data fields for Business plan
      if (colId === 'openHours' || colId === 'uptime' || colId === 'meanDistance' || colId === 'densityScore') {
        return false;
      }
  ...
  const downloadCsv = (filename: string) => {
    // Build headers - Business plan doesn't include premium fields
      ...
      <section className="flex flex-col items-center justify-center h-2/3 w-screen p-2">
        ...
        <div className="flex lg:flex-row md:flex-row flex-col items-center justify-evenly w-full p-4 relative">
          ...
          <form onSubmit={handleFormSubmit} className={error?.type === 'limit_exceeded' ? 'pointer-events-none opacity-50' : ''}>
            ...
            <div className="mb-4">
              <label htmlFor="name" className="block text-lg font-semibold text-text text-left">Business Name <span className="italic text-sm">(Required)</span></label>
              <input placeholder="Nawaabs" type="name" id="name" name="name" onChange={handleFormChange} required className="mt-1 block w-full px-3 py-2 bg-foreground border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-slate-500" />
            </div>
            ...
            <Button variant="outline" type="submit" disabled={!formData.type || !formData.city || !formData.country || !formData.name} className="px-6 py-3 bg-foreground mx-auto border-2 border-border rounded-md text-lg font-semibold hover:bg-slate-700 hover:scale-95 transition duration-300">Search</Button>
          </form>
          <div>
